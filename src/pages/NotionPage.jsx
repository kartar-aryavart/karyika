    ...
    <div style={{border:"1px solid var(--border)",borderRadius:12,overflow:"hidden",marginBottom:4}}>
            ...
            return (
              <div key={status} style={{minWidth:180,flexShrink:0}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {colRows.map(row=>(
                    <div key={row.id} style={{background:"var(--input-bg)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:9,padding:"9px 11px",fontSize:12}}>
      ...
      {view==="gallery"&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,padding:"12px"}}>
          {rows.map(row=>(
            <div key={row.id} style={{background:"var(--input-bg)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:"12px",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.borderColor="rgba(255,107,53,0.3)"} onMouseLeave={e=>e.currentTarget.style.borderColor="rgba(255,255,255,0.06)"}>
...
function Block({ block, index, blocks, onUpdate, onDelete, onAdd, depth=0, allPages, onOpenPage }) {
  ...
  const renderContent=()=>{
    switch(block.type){
      ...
      case "image": return <div style={{borderRadius:10,overflow:"hidden",border:"1px solid rgba(255,255,255,0.06)"}}>{block.content?<img src={block.content} alt="" style={{width:"100%",maxHeight:420,objectFit:"cover",display:"block"}} onError={e=>e.target.style.display="none"}/>:<div style={{padding:"24px",textAlign:"center",background:"var(--surface2)"}}><div style={{fontSize:28,marginBottom:8}}>🖼</div><input value={block.content||""} onChange={e=>onUpdate(block.id,{content:e.target.value})} placeholder="Paste image URL..." style={{...inp,width:"80%",textAlign:"center"}}/></div>}</div>;
      case "video": return <div style={{borderRadius:10,overflow:"hidden",border:"1px solid rgba(255,255,255,0.06)",background:"var(--surface2)"}}>{block.content?.includes("youtube")?<iframe src={block.content.replace("watch?v=","embed/").replace("youtu.be/","www.youtube.com/embed/")} width="100%" height="315" frameBorder="0" allowFullScreen style={{display:"block"}}/>:<div style={{padding:"24px",textAlign:"center"}}><div style={{fontSize:28,marginBottom:8}}>▶️</div><input value={block.content||""} onChange={e=>onUpdate(block.id,{content:e.target.value})} placeholder="Paste YouTube URL..." style={{...inp,width:"80%",textAlign:"center"}}/></div>}</div>;
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:"var(--bg)"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"9px 18px",borderBottom:"1px solid var(--border)",flexShrink:0,background:"var(--topbar-bg)",backdropFilter:"blur(20px)"}}>
        <button onClick={onBack} style={{padding:"5px 12px",background:"var(--surface3)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:8,color:"var(--text2)",cursor:"pointer",fontFamily:"inherit",fontSize:12}}>← Back</button>
        ...
        {[
          ...
          <button key={btn.icon} onClick={btn.action} title={btn.title} style={{padding:"5px 8px",background:btn.active?"rgba(255,107,53,0.1)":"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:8,color:btn.active?"#FF6B35":"#6B7280",cursor:"pointer",fontSize:13}}>{btn.icon}</button>
        ))}
      </div>
      {/* Share bar */}
      {showShare&&(
        ...
        <div style={{flex:1,overflowY:"auto"}}>
          ...
          <div style={{maxWidth:fullWidth?"100%":760,margin:"0 auto",padding:cover?"20px 60px 0":"60px 60px 0"}}>
            ...
            <div style={{display:"flex",gap:10,marginBottom:12,position:"relative"}}>
              ...
              {!cover&&<button onClick={()=>setShowCoverPicker(s=>!s)} style={{alignSelf:"flex-end",padding:"4px 10px",background:"var(--surface3)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:8,color:"var(--text3)",cursor:"pointer",fontSize:11,fontFamily:"inherit",marginBottom:4}}>+ Cover</button>}
            </div>
  ...
  return (
    <div style={{maxWidth:1100,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:22}}>
        ...
        <div style={{display:"flex",gap:4,background:"var(--input-bg)",borderRadius:9,padding:3,border:"1px solid rgba(255,255,255,0.06)"}}>
