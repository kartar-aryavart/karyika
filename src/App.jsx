...
export default function App() {
  ...
  useEffect(() => {
    ...
    setDataLoading(true);
    const unsubTasks    = subscribeToTasks(user.uid, d => { setTasks(d); setDataLoading(false); });
    ...
    return () => { unsubTasks(); unsubHabits(); unsubNotes(); unsubProjects(); unsubGoals(); };
  }, [user]);
  ...
  const PAGE_TITLES = {
    ...
    calendar: t("calendar"), timer: t("focusTimer"), notes: t("notes"),
    projects: t("projects"), goals: "Goals & OKRs", settings: t("settings"), admin: "🛡️ Admin",
  };

  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
      <Sidebar page={page} setPage={setPage} dark={dark} setDark={setDark} pendingCount={pendingCount} open={sidebarOpen} setOpen={setSidebarOpen} isAdmin={isAdmin} />
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        ...
        <div style={{ position: "sticky", top: 0, zIndex: 50, background: "var(--bg)", borderBottom: "1px solid var(--border)", padding: "13px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", backdropFilter: "blur(8px)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {isAdmin && page !== "admin" && (
              <button onClick={() => setPage("admin")} style={{ padding: "5px 12px", border: "1.5px solid var(--border)", borderRadius: 20, background: "var(--surface2)", cursor: "pointer", fontSize: 11, fontWeight: 700, color: "var(--text2)", fontFamily: "var(--font-body)" }}>🛡️ Admin</button>
            )}
