...
const BADGES = {
  ...
  "NOTION":{bg:"rgba(229,231,235,0.08)", color:"#9CA3AF"},
};
...
export default function Sidebar({ page, setPage, dark, setDark, pendingCount, open, setOpen, isAdmin }) {
  ...
  return (
    <>
      {open&&isMobile&&<div onClick={()=>setOpen(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:99,backdropFilter:"blur(4px)"}}/>}
      <nav style={{width:220,display:"flex",flexDirection:"column",zIndex:100,flexShrink:0,background:"linear-gradient(180deg,#08080D 0%,#0A0A14 100%)",borderRight:"1px solid rgba(255,255,255,0.04)",position:isMobile?"fixed":"relative",top:0,left:0,bottom:0,transform:isMobile?(open?"translateX(0)":"translateX(-100%)"):"none",transition:"transform 0.22s cubic-bezier(0.4,0,0.2,1)",boxShadow:isMobile&&open?"0 0 60px rgba(0,0,0,0.9)":"none"}}>
        {/* Logo */}
        <div style={{padding:"16px 14px 12px",borderBottom:"1px solid rgba(255,255,255,0.04)",flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:32,height:32,borderRadius:8,overflow:"hidden",flexShrink:0,background:"linear-gradient(135deg,rgba(255,107,53,0.15),rgba(139,92,246,0.1))",display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid rgba(255,107,53,0.15)"}}>
              <img src="/logo.png" alt="K" style={{width:32,height:32,objectFit:"contain"}} onError={e=>{e.target.style.display="none";e.target.parentElement.innerHTML="<span style='font-size:16px'>⚡</span>";}}/>
            </div>
            <div>
          ...
          {NAV(isAdmin).map(section=>(
            <div key={section.title} style={{marginBottom:2}}>
              ...
              {section.items.map(item=>{
                const active=page===item.id, hovered=hov===item.id;
                const bs=BADGES[item.badge]||{};
                return (
                  <button key={item.id} onClick={()=>{setPage(item.id);setOpen(false);}} onMouseEnter={()=>setHov(item.id)} onMouseLeave={()=>setHov(null)}
                    style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"6px 9px",border:"none",borderRadius:8,cursor:"pointer",fontFamily:"inherit",textAlign:"left",background:active?`${item.color}12`:hovered?"rgba(255,255,255,0.03)":"transparent",transition:"all 0.12s",position:"relative"}}>
                    ...
                    <div style={{width:24,height:24,borderRadius:6,flexShrink:0,background:active?`${item.color}18`:"rgba(255,255,255,0.03)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,boxShadow:active?`0 0 10px ${item.color}33`:"none"}}>{item.emoji}</div>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.04)",padding:"8px 9px",flexShrink:0}}>
          <div style={{display:"flex",gap:5,marginBottom:7}}>
            <button onClick={()=>setDark(d=>!d)} style={{flex:1,padding:"5px",border:"1px solid rgba(255,255,255,0.05)",borderRadius:7,background:"rgba(255,255,255,0.02)",cursor:"pointer",fontSize:12,color:"#6B7280",fontFamily:"inherit"}}>{dark?"☀️":"🌙"}</button>
            <button onClick={toggleLang} style={{flex:1,padding:"5px",border:"1px solid rgba(255,255,255,0.05)",borderRadius:7,background:"rgba(255,255,255,0.02)",cursor:"pointer",fontSize:10,color:"#6B7280",fontFamily:"inherit",fontWeight:700}}>{lang==="en"?"हिं":"EN"}</button>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"5px 7px",borderRadius:8,background:"rgba(255,255,255,0.02)"}}>
            <div style={{width:26,height:26,borderRadius:"50%",background:"linear-gradient(135deg,#FF6B35,#8B5CF6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,color:"#fff",flexShrink:0}}>{(user?.displayName||user?.email||"U")[0].toUpperCase()}</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:11,fontWeight:600,color:"#C9D1D9",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user?.displayName||user?.email?.split("@")[0]||"User"}</div>
              <div style={{fontSize:8,color:"#4B5563",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px"}}>Pro · Phase 6+7</div>
            </div>
            <button onClick={logout} style={{background:"transparent",border:"none",color:"#4B5563",cursor:"pointer",fontSize:13,lineHeight:1,transition:"color 0.15s"}} onMouseEnter={e=>e.currentTarget.style.color="#F43F5E"} onMouseLeave={e=>e.currentTarget.style.color="#4B5563"}>↩</button>
          </div>
        </div>
      </nav>
    </>
